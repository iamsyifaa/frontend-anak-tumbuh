...
const REPORTS: ReportDefinition[] = [
  {
    id: "student",
    title: "Laporan Siswa",
    description: "Rekap perkembangan siswa dalam periode terpilih.",
    allowedRoles: ["kepala_sekolah", "wali_kelas"],
    exportFormats: ["csv", "pdf"],
  },
  {
    id: "class",
    title: "Laporan Rombel / Kelas",
    description: "Rekap satu rombel atau kelas dalam scope sekolah.",
  ...
  },
  {
    ...
    description: "Rekap badge, penghargaan, dan sertifikat yang tercatat.",
    allowedRoles: ["kepala_sekolah", "wali_kelas"],
    exportFormats: ["csv", "pdf"],
  },
  {
    id: "habit",
    title: "Laporan Per Kebiasaan",
    description:
      "Rekap satu dari tujuh kebiasaan, termasuk deskripsi isian dan inisiatif.",
    allowedRoles: ["kepala_sekolah", "wali_kelas"],
    exportFormats: ["csv", "pdf"],
  },
];
...
function applyScope(user: UserProfile, filter: ReportFilter) {
  ...
  if (user.role === "kepala_sekolah" && filter.classId)
    rows = rows.filter((row) => row.classId === filter.classId);
  if (filter.studentId)
    rows = rows.filter((row) => row.id === filter.studentId);
  if (filter.search?.trim()) {
    const search = filter.search.trim().toLowerCase();
    rows = rows.filter((row) =>
      `${row.studentName} ${row.nis} ${row.className}`
        .toLowerCase()
        .includes(search),
    );
  }
...
function makeColumns(scope: ReportScope): ReportColumn[] {
  ...
  if (scope === "habit")
    return [
      ...
      { key: "habitDescription", label: "Deskripsi Isian" },
...
function makeRows(
  scope: ReportScope,
  filter: ReportFilter,
  rows: ReturnType<typeof applyScope>,
): ReportRow[] {
  if (scope !== "habit") return rows;
  const selections = filter.initiatives?.length
    ? filter.initiatives
    : (["Sadar sendiri", "Disuruh"] as InitiativeReportValue[]);
  return rows
    .map((row) => {
          ...
          row.method === "MANUAL"
            ...
            : isFilled
              ? Math.max(
                  0,
                  Math.round(
                    (row.points ?? 0) / Math.max(row.completedDays ?? 1, 1),
                  ),
                )
              : 0,
        habitExp:
          row.method === "MANUAL"
            ...
            : isFilled
              ? Math.max(
                  0,
                  Math.round(
                    (row.exp ?? 0) / Math.max(row.completedDays ?? 1, 1),
                  ),
                )
              : 0,
        habitDescription: isFilled
          ? HABIT_DESCRIPTIONS[filter.habitId ?? "habit-1"]
          : "Belum mengisi",
        initiative,
      };
    })
    .filter(
      (row) =>
        row.method === "MANUAL" ||
        !row.initiative ||
        selections.includes(row.initiative),
    );
}
...
export const reportService = {
  async getContext(user: UserProfile): Promise<ReportContext> {
    ...
    if (!reports.length)
      throw new Error(
        "Report Center hanya tersedia untuk Kepala Sekolah dan Wali Kelas.",
      );
    const allowedClassIds =
      user.role === "wali_kelas" ? [user.classId ?? ""] : ["cls-5a", "cls-8b"];
    return {
      ...
      availableClasses:
        user.role === "kepala_sekolah"
          ? [
              { id: "cls-5a", name: "5A • Cendekia" },
              { id: "cls-8b", name: "8B • Inspiratif" },
            ]
          : [],
  ...
  async getReport(
    ...
    if (user.role !== "kepala_sekolah" && user.role !== "wali_kelas")
      throw new Error("Report Center tidak tersedia untuk role ini.");
    if (
      !user.permissions.includes("read:reports") &&
    ...
    if (user.role === "wali_kelas" && filter.classId)
      throw new Error("Wali Kelas tidak memiliki filter rombel / kelas.");
    const definition = getAllowedDefinitions(user).find(
      (item) => item.id === filter.scope,
    );
    if (!definition)
      throw new Error("Anda tidak memiliki akses ke jenis laporan ini.");
    if (
      filter.scope === "habit" &&
      !HABITS.some((habit) => habit.id === filter.habitId)
    )
      throw new Error("Kebiasaan belum dipilih.");
    if (filter.scope === "class" && user.role !== "kepala_sekolah")
      throw new Error(
        "Wali Kelas tidak memiliki jenis laporan per rombel / kelas.",
      );
    const baseRows = applyScope(user, filter);
    const rows = makeRows(filter.scope, filter, baseRows);
    const achievementRows = baseRows.map((row) => ({
      id: row.id,
    ...
    const selectedHabit = HABITS.find(
      (habit) => habit.id === filter.habitId,
    )?.name;
    const selectedInitiatives = filter.initiatives?.length
      ? filter.initiatives.join(" + ")
      : "Semua inisiatif";
    return {
      ...
      title:
        filter.scope === "habit"
          ? `Laporan ${selectedHabit ?? "Kebiasaan"}`
          : definition.title,
      subtitle:
        filter.scope === "habit"
          ? `Rekap ${selectedHabit ?? "kebiasaan"} · Inisiatif: ${selectedInitiatives}.`
      ...
      totals: {
        ...
        activeDays: baseRows.reduce(
          (sum, row) => sum + (row.completedDays ?? 0),
          0,
        ),
      },
      exportAllowed: true,
  ...
  async exportReport(
    ...
    if (
      !user.permissions.includes("export:reports") &&
      !user.permissions.includes("*")
    )
