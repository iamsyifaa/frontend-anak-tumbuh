...
export const StudentDashboardAggregate: React.FC<Props> = ({ data, loading, error, onRetry, onOpenRanking, onOpenAchievements }) => {
  ...
  if (data.student.method === "MANUAL") return <section className="rounded-[2rem] border-2 border-amber-200 bg-amber-50 p-6 md:p-8"><div className="flex gap-4 items-start"><div className="w-12 h-12 rounded-2xl bg-amber-100 text-amber-700 flex items-center justify-center shrink-0"><BookOpenCheck /></div><div><h2 className="text-xl font-black text-black">Dashboard siswa Manual</h2><p className="mt-1 text-sm text-slate-600 font-semibold">Data siswa tetap tersedia di aplikasi, tetapi pengisian 7 Kebiasaan dilakukan melalui buku fisik. Dashboard tidak menyediakan jalur rekap buku Manual.</p></div></div></section>;
    ...
    <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start anim-stagger-2">
      ...
      <div className="lg:col-span-2 min-w-0 space-y-6">
        ...
        <div className="bg-white rounded-[2rem] p-5 md:p-7 shadow-xl shadow-sky-100/60 border border-white">
          <div className="flex items-start justify-between gap-4 mb-5">
            <div><p className="text-xs font-black uppercase tracking-wider text-[#3A72E3]">Aktivitas mingguan</p><h2 className="text-xl md:text-2xl font-black text-black mt-1">Perkembangan 7 Hari</h2></div>
      ...
      <div className="hidden lg:block min-w-0 space-y-6">
        ...
        <section className="bg-white rounded-[2rem] p-5 md:p-7 shadow-xl shadow-slate-100 border border-white lg:h-[360px] lg:overflow-y-auto">
          <div className="flex items-center justify-between gap-3 mb-5"><div><p className="text-xs font-black uppercase tracking-wider text-slate-400">Aktivitas</p><h2 className="text-xl font-black text-black">Riwayat Terbaru</h2></div><Clock3 className="w-5 h-5 text-slate-400" /></div>
...
const RankingBanner: React.FC<{ rankLabel?: string | null; onOpenRanking: () => void }> = ({ rankLabel, onOpenRanking }) => <section className="rounded-[2rem] p-5 md:p-7 bg-gradient-to-r from-sky-500 to-blue-600 text-white shadow-xl shadow-sky-200/60 anim-stagger-4">
  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5">
    ...
    <button onClick={onOpenRanking} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white text-[#3A72E3] px-5 py-3 text-sm font-black shadow-lg hover:-translate-y-0.5 transition">Lihat ranking <ChevronRight className="w-4 h-4" /></button>
  </div>
</section>;
...
const StreakCompactCard: React.FC<{ data: Aggregate }> = ({ data }) => <div className="h-full min-h-[156px] bg-gradient-to-br from-orange-50 to-amber-50 rounded-[1.6rem] p-4 border-2 border-amber-100 shadow-lg shadow-amber-100/40 flex flex-col">
  <div className="flex items-center gap-1.5"><Flame className="w-4 h-4 text-orange-500" /><h2 className="text-sm font-black text-black">Streak</h2></div>
  ...
  <div className="mt-auto pt-2 flex items-center justify-between gap-2"><p className="text-[8px] font-black uppercase text-slate-500">Rekor</p><p className="text-[11px] font-black text-black">{data.streak.best} hari</p></div>
</div>;
...
const StreakCard: React.FC<{ data: Aggregate }> = ({ data }) => <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-[2rem] p-5 md:p-7 border-2 border-amber-100 shadow-xl shadow-amber-100/50">
  <div className="flex items-center gap-2"><Flame className="text-orange-500" /><h2 className="text-xl font-black text-black">Streak</h2></div>
...
const MetricCard: React.FC<{ icon: React.ReactNode; label: string; value: string; helper: string; tone: "amber" | "violet" | "sky" }> = ({ icon, label, value, helper, tone }) => {
  const tones = { amber: "bg-amber-50 text-amber-700", violet: "bg-violet-50 text-violet-700", sky: "bg-sky-50 text-[#3A72E3]" };
  return <div className="bg-white rounded-[1.6rem] p-4 md:p-5 shadow-lg shadow-slate-100 border border-slate-100 min-w-0 min-h-[156px] h-full transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
    ...
    <p className="text-2xl md:text-3xl font-black text-black truncate mt-0.5">{value}</p>
...
const SummaryPanel: React.FC<{ title: string; icon: React.ReactNode; value: string; description: string; onClick: () => void; compact?: boolean }> = ({ title, icon, value, description, onClick, compact = false }) => <button onClick={onClick} className={`text-left bg-white rounded-[1.4rem] sm:rounded-[1.75rem] p-2.5 sm:p-4 md:p-5 border-2 border-slate-100 shadow-lg hover:-translate-y-1 hover:shadow-xl hover:border-sky-200 transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-sky-100 min-w-0 ${compact ? "min-h-[112px] sm:min-h-[130px]" : ""}`}>
  <div className="flex items-center justify-between gap-1"><div className="flex items-center gap-1.5 min-w-0">{icon}<h3 className="font-black text-black text-[11px] sm:text-sm truncate">{title}</h3></div><ChevronRight className="w-3.5 h-3.5 text-slate-400 shrink-0 hidden sm:block" /></div>
  <p className="text-xl sm:text-3xl font-black text-black mt-2.5 sm:mt-4 leading-none">{value}</p><p className="text-[8px] sm:text-[11px] leading-tight text-slate-500 font-bold mt-1 break-words">{description}</p>
</button>;

const SmallStat: React.FC<{ label: string; value: string }> = ({ label, value }) => <div className="rounded-2xl bg-white/70 border border-white p-3"><p className="text-[10px] uppercase font-black text-slate-500">{label}</p><p className="text-lg font-black text-black mt-0.5">{value}</p></div>;
const HistoryRow: React.FC<{ item: Aggregate["history"][number] }> = ({ item }) => <div className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2"><BookOpenCheck className="w-4 h-4 text-sky-500 shrink-0" /><p className="font-black text-black truncate">{item.habitName}</p></div><p className="text-xs text-slate-500 mt-1 font-semibold">{item.dateLabel} · {item.initiative}</p></div><div className="flex items-center gap-3 text-xs font-black"><span className="rounded-full bg-amber-50 text-amber-700 px-3 py-1.5">+{item.pointsAwarded} Poin</span><span className="rounded-full bg-violet-50 text-violet-700 px-3 py-1.5">+{item.expAwarded} EXP</span></div></div>;
...
const DashboardError: React.FC<{ message: string; onRetry: () => void }> = ({ message, onRetry }) => <section className="rounded-[2rem] bg-white border-2 border-red-100 p-10 text-center shadow-lg"><div className="w-12 h-12 mx-auto rounded-2xl bg-red-50 text-red-500 flex items-center justify-center"><RefreshCw /></div><h2 className="font-black text-black mt-4">Dashboard belum dapat dimuat</h2><p className="text-sm text-slate-500 mt-1 max-w-md mx-auto">{message}</p><button onClick={onRetry} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-[#3A72E3] text-white px-4 py-2.5 text-sm font-black"><RefreshCw className="w-4 h-4" /> Coba lagi</button></section>;
