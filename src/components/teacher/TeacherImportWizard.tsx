...
export const TeacherImportWizard: React.FC<Props> = ({ user, schoolId, groups, onClose, onCommitted }) => {
  ...
  const parseFile = async (file: File) => {
    ...
    try {
      ...
      const mapped: ImportTeacherRow[] = parsed.rows.map((row, index) => ({
        ...
        username: getCell(row, parsed.headers, ["username", "user name", "akun"]),
  ...
  const commit = async () => {
    ...
    try { const response = await teacherService.commitImport(user, schoolId, rows); setResult(response.imported); setStep("commit"); onCommitted(); }
