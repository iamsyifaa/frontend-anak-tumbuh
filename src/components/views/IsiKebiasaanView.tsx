...
import { AlertCircle, ArrowLeft, Check, CheckCircle2, Clock, HelpCircle, Loader2, Sparkles, Lock, RefreshCw, Lightbulb, MessageCircle } from "lucide-react";
...
export const IsiKebiasaanView: React.FC<IsiKebiasaanViewProps> = () => {
  ...
  const [commentThreads, setCommentThreads] = useState<Record<string, StudentActivityComment[]>>({});
  const [unreadTeacherComments, setUnreadTeacherComments] = useState<Set<string>>(new Set());
  const [openCommentHabitId, setOpenCommentHabitId] = useState<string | null>(null);
  ...
  const load = async () => {
    ...
    try {
      ...
      const grouped = aggregate.teacherComments.reduce<Record<string, StudentActivityComment[]>>((acc, comment) => {
        (acc[comment.habitId] ??= []).push(comment);
        return acc;
      }, {});
      setCommentThreads(grouped);
      setUnreadTeacherComments(new Set(aggregate.teacherComments.map((comment) => comment.habitId)));
    } catch (err) {
  ...
  const scorePercent = totalCount ? Math.round((completedCount / totalCount) * 100) : 0;
  ...
  const submitComment = async (habitId: string) => {
    ...
    try {
      ...
      const parentCommentId = thread.length ? thread[thread.length - 1].id : undefined;
      const created = await studentDashboardService.addActivityComment({ user, habitId, message: commentDraft, parentCommentId });
      setCommentThreads((current) => ({ ...current, [habitId]: [...(current[habitId] ?? []), created] }));
      setCommentDraft("");
    } catch (err) {
  ...
  if (selectedHabit) {
    return <>
      <div className="w-full max-w-3xl mx-auto space-y-6 pb-12">
        <button onClick={back} disabled={submitting} className="flex items-center gap-2 text-slate-500 hover:text-sky-600 font-bold bg-white px-4 py-2 rounded-full shadow-sm disabled:opacity-50"><ArrowLeft className="w-4 h-4" /> Kembali ke Laporan Harian</button>
      ...
      {showSuccessModal && <SuccessModal onClose={() => { if (successTimerRef.current) window.clearTimeout(successTimerRef.current); setShowSuccessModal(false); back(); }} />}
    </>;
  }

  const renderHabitCard = (habit: Habit, index: number) => {
    const isDone = completedIds.includes(habit.id);
    const cardBg = ["bg-amber-50", "bg-emerald-50", "bg-sky-50", "bg-orange-50", "bg-violet-50", "bg-rose-50", "bg-teal-50"][index % 7];
    const badgeBg = ["bg-amber-400", "bg-emerald-400", "bg-sky-400", "bg-orange-400", "bg-violet-400", "bg-rose-400", "bg-teal-400"][index % 7];
    const isSleepHabit = habit.name.toLowerCase().includes("tidur");
    const thread = commentThreads[habit.id] ?? [];
    const hasTeacherComment = thread.some((comment) => comment.authorRole === "wali_kelas");
    const hasUnreadTeacherComment = unreadTeacherComments.has(habit.id) && hasTeacherComment;
    const isCommentsOpen = openCommentHabitId === habit.id;
    return <div key={habit.id} className={`relative min-w-0 rounded-3xl ${cardBg} p-4 pt-6 shadow-md shadow-sky-100/50 transition-all duration-300 group overflow-visible ${isCommentsOpen ? "z-10" : "z-0"}`}>
      <button type="button" onClick={() => openHabit(habit)} disabled={isDone} className={`w-full text-left ${isDone ? "cursor-not-allowed" : "cursor-pointer"}`}>
        <span className={`absolute top-3 left-3 z-20 w-7 h-7 rounded-full ${badgeBg} text-white text-xs font-black flex items-center justify-center border-2 border-white shadow-sm`}>{index + 1}</span>
        {isDone && <span className="absolute top-3 right-3 z-20 w-7 h-7 rounded-full bg-emerald-500 text-white flex items-center justify-center border-2 border-white shadow-sm"><Check className="w-4 h-4" /></span>}
        <div className={`flex items-center justify-center ${isSleepHabit ? "h-36" : "h-32"}`}><img src={`/image/${encodeURIComponent(illustrationFor(habit.name))}.png`} alt={habit.name} className={`${isSleepHabit ? "h-36" : "h-full"} w-auto object-contain transition-transform duration-300 ${isDone ? "opacity-50 grayscale" : "group-hover:scale-105"}`} /></div>
        <div className="bg-white/75 backdrop-blur-sm rounded-2xl p-3 mt-3 -mx-1"><p className="text-[10px] text-slate-500 font-bold uppercase tracking-wide mb-0.5 truncate">Kebiasaan #{index + 1}</p><h4 className="text-sm font-extrabold text-slate-800 leading-snug break-words mb-1.5">{habit.name}</h4><p className="text-xs text-slate-500 leading-relaxed line-clamp-2">{habit.description}</p><div className="mt-3 flex items-center justify-between gap-2"><span className={`inline-flex items-center gap-1 text-xs font-bold ${isDone ? "text-emerald-700" : "text-sky-600"}`}>{isDone ? <><Check className="w-3.5 h-3.5" /> Terkunci</> : <><Clock className="w-3.5 h-3.5" /> Isi sekarang</>}</span><span className="font-black text-slate-600 bg-white rounded-full px-2.5 py-1 text-[10px]">{isDone ? "Sudah diisi" : "Buka form →"}</span></div></div>
      </button>
      <button type="button" onClick={() => openComments(habit.id)} className="relative mt-3 w-full inline-flex items-center justify-center gap-1.5 bg-white/90 border border-sky-100 text-sky-600 rounded-full px-3 py-2 text-[11px] font-extrabold hover:bg-white hover:border-sky-200 transition-colors"><MessageCircle className="w-3.5 h-3.5" /> Komentar{hasUnreadTeacherComment && <span aria-label="Komentar baru dari wali kelas" className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-rose-500 border-2 border-white shadow-md" />}</button>
      {isCommentsOpen && <div className="relative mt-3 rounded-2xl bg-white border border-sky-100 p-3 shadow-lg shadow-slate-300/20" onClick={(event) => event.stopPropagation()}><div className="flex items-center justify-between gap-2 mb-3"><div><p className="text-xs font-black text-slate-800">Komentar aktivitas</p><p className="text-[10px] text-slate-400">Siswa dan Wali Kelas dapat saling membalas.</p></div><button type="button" onClick={closeComments} className="text-[10px] font-bold text-slate-400 hover:text-slate-600">Tutup</button></div><div className="space-y-2.5 max-h-52 overflow-y-auto pr-1">{thread.length === 0 && <p className="text-[11px] text-slate-400 py-2">Belum ada komentar.</p>}{thread.map((comment) => <div key={comment.id} className={`rounded-2xl px-3 py-2.5 ${comment.authorRole === "wali_kelas" ? "bg-rose-50 border border-rose-100" : "bg-sky-50 border border-sky-100"}`}><div className="flex items-center justify-between gap-2"><p className={`text-[10px] font-black ${comment.authorRole === "wali_kelas" ? "text-rose-700" : "text-sky-700"}`}>{comment.authorRole === "wali_kelas" ? "Wali Kelas" : "Kamu"}</p><span className="text-[9px] text-slate-400">{new Intl.DateTimeFormat("id-ID", { day: "numeric", month: "short" }).format(new Date(comment.createdAt))}</span></div><p className="text-[11px] text-slate-600 leading-relaxed mt-1">{comment.message}</p></div>)}</div><div className="mt-3"><textarea value={commentDraft} onChange={(event) => setCommentDraft(event.target.value)} rows={2} maxLength={500} placeholder="Tulis komentar atau balasan..." className="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-[11px] font-medium text-slate-700 outline-none focus:border-sky-300 focus:ring-2 focus:ring-sky-100 resize-none" />{commentError && <p className="text-[10px] font-bold text-rose-600 mt-1">{commentError}</p>}<div className="flex justify-end mt-2"><button type="button" disabled={commentSubmitting || !commentDraft.trim()} onClick={() => void submitComment(habit.id)} className="rounded-full bg-sky-500 px-4 py-2 text-[10px] font-black text-white disabled:opacity-50">{commentSubmitting ? "Mengirim..." : "Kirim komentar"}</button></div></div></div>}
    </div>;
  };
  ...
  return <div className="space-y-7 animate-fade-in pb-12">
    <section className="bg-gradient-to-r from-sky-400 via-sky-500 to-blue-500 rounded-[2.5rem] p-6 md:p-8 border-4 border-white shadow-2xl text-white flex flex-col md:flex-row md:items-center justify-between gap-6">
      <div><div className="inline-flex items-center gap-2 bg-white/20 px-4 py-1.5 rounded-full text-xs font-black border border-white/30"><Sparkles className="w-4 h-4" /> 7 Kebiasaan Anak Hebat</div><h2 className="text-2xl md:text-4xl font-black mt-3">Laporan Harian 7 Kebiasaan</h2><p className="text-xs md:text-sm text-sky-100 font-semibold mt-2">Pilih kartu kebiasaan untuk mengisi laporan digital. Kartu yang selesai akan otomatis terkunci.</p></div>
    ...
    <div className="grid grid-cols-2 gap-2.5 sm:gap-3 lg:hidden items-stretch">
...
const CompactSummary: React.FC<{ percent: number }> = ({ percent }) => <div className="min-w-0 rounded-3xl p-[2px] bg-gradient-to-r from-amber-300 via-pink-300 to-sky-300 shadow-lg shadow-sky-100/50"><div className="bg-white rounded-[calc(1.5rem-2px)] p-2.5 sm:p-3 h-full"><h3 className="text-[10px] font-black text-indigo-600 mb-2">Ringkasan Hari Ini</h3><div className="flex items-center gap-2"><ProgressRing percent={percent} /><div className="min-w-0 text-center flex-1"><p className="text-xs font-extrabold text-amber-500">Tetap semangat!</p><p className="text-[9px] text-slate-500 leading-tight mt-1">Mulai isi kebiasaanmu.</p></div></div></div></div>;

const CompactPositive: React.FC<{ completedCount: number; totalCount: number }> = ({ completedCount, totalCount }) => <div className="min-w-0 bg-emerald-50 rounded-3xl p-2.5 sm:p-3 shadow-md shadow-emerald-100/60 flex flex-col items-center justify-center text-center"><img src="/image/bintang.png" alt="Bintang" className="w-11 h-11 object-contain" /><p className="text-[10px] font-extrabold text-emerald-800 mt-1">Penguatan Positif</p><p className="text-[9px] text-emerald-700 leading-tight mt-1">{completedCount}/{totalCount} kebiasaan hari ini. Terus semangat!</p></div>;

const ProgressRing: React.FC<{ percent: number }> = ({ percent }) => {
  const radius = 43;
  const circumference = 2 * Math.PI * radius;
  const dashOffset = circumference - (percent / 100) * circumference;
  return <div className="relative w-24 h-24 flex-shrink-0"><svg className="w-full h-full -rotate-90" viewBox="0 0 104 104"><circle cx="52" cy="52" r={radius} fill="none" stroke="#e2e8f0" strokeWidth="9" /><circle cx="52" cy="52" r={radius} fill="none" stroke="#10b981" strokeWidth="9" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={dashOffset} className="transition-all duration-700" /></svg><div className="absolute inset-0 flex flex-col items-center justify-center"><span className="text-2xl font-black text-slate-800">{percent}</span><span className="text-[9px] font-bold text-slate-400">Skor</span></div></div>;
};

const SuccessModal: React.FC<{ onClose: () => void }> = ({ onClose }) => (
  <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/45 backdrop-blur-sm p-4" role="dialog" aria-modal="true" aria-labelledby="habit-success-title">
    <div className="w-full max-w-sm rounded-[2rem] bg-white p-7 text-center shadow-2xl border-4 border-white animate-fade-in">
      <div className="mx-auto w-20 h-20 rounded-full bg-emerald-100 flex items-center justify-center shadow-inner"><CheckCircle2 className="w-11 h-11 text-emerald-500" /></div>
      <h2 id="habit-success-title" className="text-2xl font-black text-slate-800 mt-5">Selesai!</h2>
      <p className="text-sm text-slate-500 mt-2 leading-relaxed">Kebiasaan berhasil disimpan dan otomatis dikunci. Kamu akan kembali ke Laporan Harian.</p>
      <button onClick={onClose} className="mt-6 w-full rounded-2xl bg-sky-500 text-white py-3 text-sm font-black hover:bg-sky-600 transition-colors">Kembali sekarang</button>
    </div>
  </div>
);
