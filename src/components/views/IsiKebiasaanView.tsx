...
export const IsiKebiasaanView: React.FC<IsiKebiasaanViewProps> = () => {
  ...
  const renderHabitCard = (habit: Habit, index: number) => {
    ...
    return <div key={habit.id} className={`relative min-w-0 rounded-3xl ${cardBg} p-2.5 pt-5 sm:p-4 sm:pt-6 shadow-md shadow-sky-100/50 transition-all duration-300 group overflow-visible ${isCommentsOpen ? "z-10" : "z-0"}`}>
      <button type="button" onClick={() => openHabit(habit)} disabled={isDone} className={`w-full text-left ${isDone ? "cursor-not-allowed" : "cursor-pointer"}`}>
        ...
        <div className={`flex items-center justify-center ${isSleepHabit ? "h-24 sm:h-36" : "h-24 sm:h-32"}`}><img src={`/image/${encodeURIComponent(illustrationFor(habit.name))}.png`} alt={habit.name} className={`${isSleepHabit ? "h-24 sm:h-36" : "h-24 sm:h-full"} w-auto object-contain transition-transform duration-300 ${isDone ? "opacity-50 grayscale" : "group-hover:scale-105"}`} /></div>
        <div className="bg-white/75 backdrop-blur-sm rounded-xl sm:rounded-2xl p-2.5 sm:p-3 mt-2 sm:mt-3 -mx-0.5 sm:-mx-1"><p className="text-[9px] sm:text-[10px] text-slate-500 font-bold uppercase tracking-wide mb-0.5 truncate">Kebiasaan #{index + 1}</p><h4 className="text-[11px] sm:text-sm font-extrabold text-slate-800 leading-tight sm:leading-snug break-words mb-1">{habit.name}</h4><p className="hidden sm:block text-xs text-slate-500 leading-relaxed line-clamp-2">{habit.description}</p><div className="mt-2 sm:mt-3 flex items-center justify-between gap-1.5 sm:gap-2"><span className={`inline-flex items-center gap-1 text-[9px] sm:text-xs font-bold ${isDone ? "text-emerald-700" : "text-sky-600"}`}>{isDone ? <><Check className="w-3.5 h-3.5" /> Terkunci</> : <><Clock className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> Isi sekarang</>}</span><span className="font-black text-slate-600 bg-white rounded-full px-2 py-1 text-[8px] sm:text-[10px] whitespace-nowrap">{isDone ? "Sudah diisi" : "Buka form →"}</span></div></div>
      </button>
      <button type="button" onClick={() => openComments(habit.id)} className="relative mt-2 sm:mt-3 w-full inline-flex items-center justify-center gap-1 bg-white/90 border border-sky-100 text-sky-600 rounded-full px-2 sm:px-3 py-1.5 sm:py-2 text-[9px] sm:text-[11px] font-extrabold hover:bg-white hover:border-sky-200 transition-colors"><MessageCircle className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> Komentar{hasUnreadTeacherComment && <span aria-label="Komentar baru dari wali kelas" className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-rose-500 border-2 border-white shadow-md" />}</button>
