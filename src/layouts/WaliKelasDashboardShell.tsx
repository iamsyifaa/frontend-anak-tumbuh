      ...
      >
        <div className="flex h-[72px] items-center justify-between border-b border-white/10 px-5">
          ...
          >
            ...
            {!collapsed && <span className="text-lg font-black tracking-tight text-white">anaktumbuh.id</span>}
          </button>
          <button
            type="button"
            onClick={closeMobile}
        ...
        {!collapsed && (
          <div className="mx-4 mt-5 rounded-2xl bg-[#203A5B] p-4 text-white shadow-lg shadow-black/20 ring-1 ring-white/10">
            <p className="text-[10px] font-black uppercase tracking-[0.16em] text-white/70">Workspace</p>
            <p className="mt-1 text-sm font-extrabold">Wali Kelas</p>
            <p className="mt-1 truncate text-[11px] text-white/70">{user?.name}</p>
          </div>
        )}

        <nav className="mt-6 flex-1 overflow-y-auto px-3 pb-4">
          {!collapsed && (
            <p className="px-3 pb-3 text-[10px] font-black uppercase tracking-[0.16em] text-white/60">Menu utama</p>
          )}
          <div className="space-y-1.5">
            {items.map((item) => {
              const Icon = item.icon;
              const active = location.pathname === item.to ||
                (item.to !== "/dashboard/walikelas" && location.pathname.startsWith(item.to));
              return (
                <NavLink
                  key={item.to}
                  to={item.to}
                ...
                >
                  <Icon className={`h-5 w-5 shrink-0 ${active ? "text-[#203A5B]" : "text-white/60 group-hover:text-white"}`} />
      ...
      <div className={`min-h-screen transition-[padding] duration-300 ${collapsed ? "lg:pl-[82px]" : "lg:pl-[260px]"}`}>
        <header className="sticky top-0 z-20 flex h-[72px] items-center justify-between border-b border-[#203A5B]/10 bg-white/95 px-4 shadow-sm backdrop-blur sm:px-6">
          <div className="flex items-center gap-3">
            <button type="button" onClick={() => setMobileOpen(true)} className="rounded-xl border border-slate-200 p-2 text-slate-600 lg:hidden" aria-label="Buka navigasi">
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-sm font-extrabold text-slate-800">Wali Kelas</p>
            </div>
          </div>
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button type="button" onClick={() => setMobileOpen(true)} className="grid h-10 w-10 place-items-center rounded-full bg-[#203A5B] text-white font-black lg:hidden" aria-label="Profil">
              {user?.name?.slice(0, 1) ?? "W"}
            </button>
            {hasPermission("read:reports") && (
              <button type="button" onClick={() => navigate("/dashboard/walikelas/reports")} className="hidden rounded-xl border border-slate-200 p-2 text-slate-500 hover:bg-slate-50 sm:block" title="Report Center">
                <FileBarChart2 className="h-5 w-5" />
              </button>
            )}
            <button type="button" onClick={() => navigate("/dashboard/walikelas")} className="rounded-xl border border-slate-200 p-2 text-slate-500 hover:bg-slate-50" title="Monitoring Rombel">
