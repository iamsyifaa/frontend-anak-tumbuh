  ...
  const items = useMemo(
    () =>
      navItems.filter((item) => {
        ...
        return (
          roleAllowed && (!item.permission || hasPermission(item.permission))
        );
      }),
    [hasPermission, user],
  );
    ...
    <div className="min-h-screen bg-[#f5f3f8] text-slate-800">
      ...
      >
          ...
          >
            ...
            {!collapsed && (
              <span className="text-lg font-black tracking-tight text-white">
                anaktumbuh<span className="text-[#A4C1FD]">.id</span>
              </span>
            )}
          </button>
          <button
        ...
        <nav className="mt-6 flex-1 overflow-y-auto px-3 pb-4">
          {!collapsed && (
            <p className="px-3 pb-3 text-[10px] font-black uppercase tracking-[0.16em] text-[#A4C1FD]">
              Menu utama
            </p>
          )}
          <div className="space-y-1.5">
            {items.map((item) => {
              ...
              const itemPath =
                user?.role === "kepala_sekolah"
                  ? item.to.replace("/dashboard/admin", "/dashboard/kepsek")
                  : item.to;
              const active =
              ...
              return (
                <NavLink
                  ...
                  className={`group flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-bold transition ${
                    active
                      ? "bg-[#2F4D73] text-white shadow-sm"
                      : "text-[#D7EFFF] hover:bg-white/10 hover:text-white"
                  } ${collapsed ? "justify-center" : ""}`}
                >
                  <Icon
                    className={`h-5 w-5 shrink-0 ${active ? "text-white" : "text-[#A4C1FD] group-hover:text-white"}`}
                  />
        ...
        <div className="border-t border-white/10 p-3">
          <button
            ...
            className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-bold text-[#FF8A9A] transition hover:bg-white/10 ${collapsed ? "justify-center" : ""}`}
          >
        ...
        <header className="sticky top-0 z-20 flex h-[72px] items-center justify-between border-b border-slate-100 bg-white/95 px-4 shadow-sm backdrop-blur sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:block">
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#3A72E3]">
                anaktumbuh.id
              </p>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              type="button"
              onClick={() => setMobileOpen(true)}
              className="grid h-10 w-10 place-items-center rounded-full bg-[#A4C1FD] text-[#203A5B] font-black lg:hidden"
              aria-label="Profil"
            >
