...
import { AlertCircle, CheckCircle2, FileSpreadsheet,
  KeyRound, Plus, QrCode, RefreshCw, Search, ShieldAlert, Users, GraduationCap, Archive } from "lucide-react";
...
export const StudentManagementPage: React.FC = () => {
  ...
  const filteredStudents = useMemo(() => students.filter((student) => student.status !== "graduated").filter((student) => {
  ...
  return <div className="min-h-full space-y-5">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      ...
      <div className="flex flex-wrap gap-2">{user?.role === "kepala_sekolah" && <button onClick={() => window.location.assign("/dashboard/kepsek/students/archive")} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-black text-slate-700"><Archive className="h-4 w-4"/>Arsip Siswa</button>}<button onClick={() => window.location.assign(user?.role === "super_admin" ? "/dashboard/admin/student-accounts" : "/dashboard/kepsek/student-accounts")} className="inline-flex items-center gap-2 rounded-xl border border-violet-200 bg-violet-50 px-3 py-2.5 text-xs font-black text-violet-700"><KeyRound className="h-4 w-4" />Akun & QR</button>{canWrite && <button onClick={() => setDialog("placement")} className="inline-flex items-center gap-2 rounded-xl border border-sky-200 bg-sky-50 px-3 py-2.5 text-xs font-black text-sky-700"><GraduationCap className="h-4 w-4" />Kenaikan / Penempatan</button>}<button onClick={() => void load()} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-black text-slate-700"><RefreshCw className="h-4 w-4" />Refresh</button>{canImport && <button onClick={() => setDialog("import")} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-black text-white"><FileSpreadsheet className="h-4 w-4" />Import Excel</button>}{canWrite && <button onClick={() => setDialog("student")} className="inline-flex items-center gap-2 rounded-xl bg-sky-600 px-4 py-2.5 text-xs font-black text-white"><Plus className="h-4 w-4" />Tambah Siswa</button>}</div>
    </div>
