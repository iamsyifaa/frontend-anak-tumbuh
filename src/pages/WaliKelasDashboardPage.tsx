...
export const WaliKelasDashboardPage: React.FC = () => {
  ...
  return <div className="min-h-full bg-[#f6fbff] p-4 md:p-8">
    ...
    {selected && <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/40" onMouseDown={(e)=>{if(e.currentTarget===e.target)setSelected(null)}}><aside className="h-full w-full max-w-2xl overflow-y-auto bg-white shadow-2xl"><div className="sticky top-0 z-20 flex items-center justify-between border-b bg-white px-5 py-4"><div><h2 className="font-extrabold">Monitoring Siswa</h2><p className="text-xs text-slate-500">{selected.name} · {selected.method}</p></div><button onClick={()=>setSelected(null)} className="rounded-full p-2 hover:bg-slate-100"><X className="h-5 w-5"/></button></div><div className="space-y-6 p-5">
      ...
      <section><h3 className="font-extrabold">Aktivitas 7 Kebiasaan</h3>{selected.method === "MANUAL" ? <div className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">Siswa Manual menggunakan buku fisik. Tidak ada input atau rekap buku manual melalui aplikasi oleh Wali Kelas.</div> : <div className="mt-3 grid gap-2 sm:grid-cols-2">{selected.habits.map(h=><div key={h.id} className="rounded-xl border p-3"><div className="flex justify-between gap-2"><span className="text-sm font-bold">{h.name}</span><span className={`text-xs font-bold ${h.status === "done" ? "text-emerald-600" : "text-slate-400"}`}>{h.status === "done" ? "Selesai" : "Belum"}</span></div>{h.indicatorLabel && <p className="mt-1 text-xs text-slate-500">{h.indicatorLabel}</p>}</div>)}</div>}</section>
