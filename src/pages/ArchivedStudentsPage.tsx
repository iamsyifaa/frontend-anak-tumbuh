...
export const ArchivedStudentsPage: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setAcademicYears(context.years);
  ...
  const filtered = useMemo(() => students.filter((student) => {
    ...
    const last = student.enrollmentHistory?.[student.enrollmentHistory.length - 1];
    const matchesSearch = !q || [student.name, student.nis, student.nisn].some((value) => value?.toLowerCase().includes(q));
    const matchesYear = !graduationYearId || last?.academicYearId === graduationYearId;
    return matchesSearch && matchesYear;
  }), [graduationYearId, search, students]);
  const groupName = (id: string) => { const group = groups.find((item) => item.id === id); return group ? `${group.levelName} — ${group.rombelName}` : "—"; };
  const academicYearName = (id?: string) => academicYears.find((year) => year.id === id)?.name ?? "—";
  ...
  return <div className="space-y-5"><header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><div className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-[11px] font-black text-slate-700"><Archive className="h-3.5 w-3.5"/> Arsip Siswa</div><h1 className="mt-2 text-2xl font-black">Siswa Lulus / Arsip</h1><p className="mt-1 text-sm text-slate-500">Data siswa yang sudah lulus dipisahkan dari tabel siswa aktif. Histori tetap disimpan.</p></div><button type="button" onClick={() => void load()} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-black"><RefreshCw className="h-4 w-4"/>Refresh</button></header>
    ...
    <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-100"><div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div className="relative max-w-md flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400"/><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari nama, NISN, atau NIS..." className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-9 pr-3 text-sm outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100"/></div><div className="w-full lg:w-[250px]"><label className="mb-1.5 block text-[11px] font-black uppercase tracking-wide text-slate-500">Tahun Lulus</label><select value={graduationYearId} onChange={(e) => setGraduationYearId(e.target.value)} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-semibold text-slate-700 outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100"><option value="">Semua Tahun</option>{academicYears.map((year) => <option key={year.id} value={year.id}>{year.name}{year.status === "active" ? " — Aktif" : ""}</option>)}</select></div><span className="self-start rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600 lg:self-center">{filtered.length} siswa</span></div>
      {loading ? <div className="h-56 animate-pulse rounded-2xl bg-slate-50"/> : <div className="overflow-x-auto rounded-2xl border border-slate-100"><table className="w-full min-w-[820px] text-left text-sm"><thead className="bg-slate-50 text-[10px] uppercase tracking-wider text-slate-500"><tr><th className="px-4 py-3">Nama</th><th className="px-4 py-3">NISN/NIS</th><th className="px-4 py-3">Rombel terakhir</th><th className="px-4 py-3">Status QR</th><th className="px-4 py-3">Tahun lulus</th><th className="px-4 py-3">Tanggal arsip</th></tr></thead><tbody className="divide-y divide-slate-100">{filtered.map((student) => { const last = student.enrollmentHistory?.[student.enrollmentHistory.length - 1]; return <tr key={student.id}><td className="px-4 py-3 font-black">{student.name}</td><td className="px-4 py-3 text-xs text-slate-500">{student.nisn ?? student.nis ?? "—"}</td><td className="px-4 py-3 text-xs text-slate-500">{groupName(last?.classGroupId ?? student.classGroupId)}</td><td className="px-4 py-3"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-black text-slate-600">{student.qrStatus === "revoked" ? "Nonaktif" : student.qrStatus}</span></td><td className="px-4 py-3 text-xs font-semibold text-slate-600">{academicYearName(last?.academicYearId)}</td><td className="px-4 py-3 text-xs text-slate-500">{last?.recordedAt ? new Date(last.recordedAt).toLocaleDateString("id-ID") : "—"}</td></tr>; })}</tbody></table>{!filtered.length && <div className="p-10 text-center text-sm text-slate-500">Belum ada siswa yang diarsipkan.</div>}</div>}
    </section></div>;
};
