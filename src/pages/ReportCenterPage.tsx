  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm font-bold text-sky-600"><FileText className="h-4 w-4" /> Report Center</div>
        ...
        <div className="rounded-2xl bg-sky-50 px-4 py-3 text-xs font-semibold text-sky-800">Role: {user?.role.replaceAll("_", " ")}</div>
      </div>

      <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div className="mb-4 flex items-center gap-2 font-heading text-lg font-bold text-slate-900"><Filter className="h-5 w-5 text-sky-500" /> Filter laporan</div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <label className="text-sm font-bold text-slate-700">Jenis laporan<select value={filter.scope} onChange={(e) => setFilter({ ...filter, scope: e.target.value as ReportScope, studentId: undefined })} className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 font-medium outline-none focus:border-sky-400">{availableScopes.map((scope) => <option key={scope} value={scope}>{scope === "student" ? "Siswa" : scope === "class" ? "Rombel / Kelas" : scope === "school" ? "Sekolah" : "Pencapaian"}</option>)}</select></label>
          <label className="text-sm font-bold text-slate-700">Periode<select value={filter.periodPreset} onChange={(e) => updatePreset(e.target.value as ReportFilter["periodPreset"])} className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 font-medium outline-none focus:border-sky-400"><option value="this_week">Minggu ini</option><option value="this_month">Bulan ini</option><option value="this_term">Semester / periode</option><option value="custom">Custom</option></select></label>
          <label className="text-sm font-bold text-slate-700">Mulai<input type="date" value={filter.startDate} onChange={(e) => setFilter({ ...filter, startDate: e.target.value, periodPreset: "custom" })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 font-medium outline-none focus:border-sky-400" /></label>
          <label className="text-sm font-bold text-slate-700">Sampai<input type="date" value={filter.endDate} onChange={(e) => setFilter({ ...filter, endDate: e.target.value, periodPreset: "custom" })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 font-medium outline-none focus:border-sky-400" /></label>
        </div>
        <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button onClick={runReport} disabled={loading} className="inline-flex items-center justify-center gap-2 rounded-xl bg-sky-500 px-5 py-2.5 text-sm font-extrabold text-white shadow-sm hover:bg-sky-600 disabled:opacity-60"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />{loading ? "Memuat..." : "Tampilkan laporan"}</button>
        </div>
      </div>
