...
export const WaliKelasCertificatePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (id && items.length)
      setPreview(items.find((item) => item.id === id) ?? null);
  }, [items, searchParams]);
  ...
  const print = (certificate: IssuedCertificate) => {
    const win = window.open("", "_blank", "noopener,noreferrer");
    if (!win) return;
    win.document.write(buildSvg(certificate));
    win.document.close();
    win.focus();
    win.print();
  };
  if (user?.role !== "wali_kelas")
    return (
      <div className="rounded-3xl bg-white p-10 text-center">
        <ShieldAlert className="mx-auto text-rose-500" />
        <h1 className="mt-3 font-black">Akses tidak tersedia</h1>
      </div>
    );
  return (
    <div className="space-y-5">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full bg-sky-50 px-3 py-1 text-[11px] font-black text-sky-700">
            <FileBadge className="h-4 w-4" /> Sertifikat Siswa
          </div>
          <h1 className="mt-2 text-2xl font-black">Sertifikat Rombel</h1>
          <p className="mt-1 text-sm text-slate-500">
            Unduh atau cetak sertifikat siswa untuk dibagikan dalam versi
            kertas.
          </p>
        </div>
        <button
          type="button"
          onClick={() => void load()}
          className="inline-flex items-center gap-2 self-start rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-black"
        >
        ...
        <div className="overflow-x-auto rounded-3xl bg-white shadow-sm ring-1 ring-slate-100">
          <table className="w-full min-w-[900px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {items.map((item) => (
                <tr key={item.id}>
                  <td className="px-5 py-3 font-black">{item.studentName}</td>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded-lg border border-violet-200 bg-violet-50 px-3 py-2 text-[10px] font-black text-violet-700"
                      >
                        Lihat
                      </button>
                      <button
                        onClick={() => download(item)}
                        className="inline-flex items-center gap-1 rounded-lg bg-sky-50 px-3 py-2 text-[10px] font-black text-sky-700"
                      >
                      ...
                      <button
                        onClick={() => print(item)}
                        className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-3 py-2 text-[10px] font-black"
                      >
          ...
          {!items.length && (
            <div className="p-10 text-center text-sm text-slate-500">
              Belum ada sertifikat yang diberikan kepada Wali Kelas ini.
            </div>
          )}
        </div>
      )}
      {preview && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4 backdrop-blur-sm">
          <div className="flex w-full max-w-3xl max-h-[92vh] flex-col rounded-3xl bg-white p-4 shadow-2xl sm:p-5">
            ...
            <div className="mt-4 flex min-h-0 items-center justify-center overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 p-2 sm:p-3">
              <img
                alt={`Preview sertifikat ${preview.studentName}`}
                src={`data:image/svg+xml;charset=utf-8,${encodeURIComponent(buildSvg(preview))}`}
                className="block max-h-[62vh] w-auto max-w-full rounded-xl bg-white object-contain shadow-sm"
              />
            </div>
            <div className="mt-4 flex shrink-0 flex-wrap justify-center gap-2 sm:justify-end">
              <button
                type="button"
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-black"
              >
              ...
              <button
                ...
                className="rounded-xl px-4 py-2.5 text-xs font-black text-slate-600"
              >
                Tutup
              </button>
            </div>
