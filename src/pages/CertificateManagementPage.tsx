...
import { Award, CheckCircle2, FilePlus2, Filter, ShieldAlert, UserCheck } from "lucide-react";
...
export const CertificateManagementPage: React.FC = () => {
  ...
  const activeStudents = useMemo(() => context?.studentOptions ?? [], [context]);
  ...
  const issue = async () => {
    if (!user || !schoolId || !selectedTemplateId || !selectedCount) return;
    setIssuing(true);
    setError("");
    try {
  ...
  return <div className="space-y-6">
    <header>
      ...
      <p className="mt-1 max-w-3xl text-sm font-medium text-slate-500">Template dan penerbitan dikelola admin. UI ini hanya mengirim intent; validasi penerima, template aktif, scope sekolah, dan pembuatan final tetap menjadi tanggung jawab backend.</p>
    </header>
    ...
    <section className="grid gap-6 xl:grid-cols-[.9fr_1.1fr]">
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between gap-3"><div><h2 className="font-black text-slate-900">Template sertifikat</h2><p className="mt-1 text-xs text-slate-500">Template harus dikelola sebagai konfigurasi versioned.</p></div><FilePlus2 className="h-5 w-5 text-violet-500" /></div>
        <div className="mt-4 space-y-3">
          ...
          <label className="block text-xs font-black text-slate-600">Nama template baru<input className={inputClass} value={templateName} onChange={(e) => setTemplateName(e.target.value)} placeholder="Contoh: Sertifikat Kemandirian Bulanan" /></label>
          <button type="button" disabled={creatingTemplate || !templateName.trim()} onClick={() => void createTemplate()} className="w-full rounded-xl bg-violet-600 px-4 py-2.5 text-xs font-black text-white disabled:opacity-50">{creatingTemplate ? "Menyimpan..." : "Buat template"}</button>
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <button type="button" disabled={issuing || !selectedTemplateId || !selectedCount || !periodLabel.trim()} onClick={() => void issue()} className="mt-4 w-full rounded-xl bg-sky-600 px-4 py-3 text-sm font-black text-white disabled:opacity-50">{issuing ? "Membuat sertifikat..." : `Buat & berikan ${selectedCount || ""} sertifikat`}</button>
      </div>
    </section>
