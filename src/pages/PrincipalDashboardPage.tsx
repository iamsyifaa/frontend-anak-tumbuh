...
export const PrincipalDashboardPage: React.FC = () => {
  ...
  const stats = [
    ...
    stat("Tahun ajaran", years.filter(y => y.status === "active").length || years.length, CalendarDays, "bg-blue-50 text-blue-600"),
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-6 lg:grid-cols-[1.45fr_.85fr]">
      <article className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-100 sm:p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-violet-500">Struktur sekolah</p><h2 className="mt-1 text-xl font-black text-slate-900">Rombel yang terdaftar</h2></div><button onClick={() => navigate("/dashboard/kepsek?tab=class-group")} className="inline-flex items-center gap-1 rounded-xl bg-violet-50 px-3 py-2 text-xs font-extrabold text-violet-700">Kelola <ArrowRight className="h-4 w-4"/></button></div><div className="mt-5 grid gap-3 sm:grid-cols-2">{classes.slice(0,6).map(group => <div key={group.id} className="rounded-2xl border border-slate-100 bg-slate-50/70 p-4"><div className="flex items-center justify-between gap-2"><p className="font-extrabold text-slate-800">{group.levelName} {group.rombelName}</p><span className="rounded-full bg-white px-2 py-1 text-[10px] font-bold text-slate-500">Rombel</span></div></div>)}{classes.length === 0 && <div className="sm:col-span-2 rounded-2xl border border-dashed border-slate-200 p-8 text-center text-sm text-slate-500">Belum ada rombel.</div>}</div></article>
